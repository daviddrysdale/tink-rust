...
// that we correcly compute HMAC over ciphertext and associated_data.
#[derive(Debug)]
...
fn test_eta_rfc_test_vectors() {
    for v in RFC_TEST_VECTORS {
        ...
        let associated_data = hex::decode(&v.associated_data).unwrap();
...
fn test_eta_encrypt_decrypt() {
    ...
    let associated_data = b"Some data to authenticate.";

    let ciphertext = cipher
        .encrypt(&message[..], &associated_data[..])
...
fn test_eta_encrypt_decrypt_random_message() {
    ...
    for i in 0..256 {
        ...
        let associated_data = get_random_bytes(i);
...
fn test_eta_decrypt_modified_ciphertext() {
    ...
    let mut mod_ciphertext = Vec::with_capacity(ciphertext.len());
    mod_ciphertext.extend_from_slice(&ciphertext);
    for i in 0..ciphertext.len() * 8 {
        // Save the byte to be modified.
        let b = mod_ciphertext[i / 8];
        mod_ciphertext[i / 8] ^= 1 << (i % 8);
        assert_ne!(
            ciphertext, mod_ciphertext,
            "modified ciphertext shouldn't be the same as original"
        );
        cipher
            .decrypt(&mod_ciphertext, &associated_data[..])
            .expect_err(&format!(
                "successfully decrypted modified ciphertext (i = {})",
                i
            ));
        // Restore the modified byte.
        mod_ciphertext[i / 8] = b;
    }

    // Modify the associated authenticated data.
    let mut mod_associated_data = Vec::with_capacity(associated_data.len());
    mod_associated_data.extend_from_slice(&associated_data[..]);
    for i in 0..associated_data.len() * 8 {
        // Save the byte to be modified.
        let b = mod_associated_data[i / 8];
        mod_associated_data[i / 8] ^= 1 << (i % 8);
        assert_ne!(
            associated_data.to_vec(),
            mod_associated_data,
            "modified associated_data shouldn't be the same as associated_data"
        );
        cipher
            .decrypt(&ciphertext, &mod_associated_data)
            .expect_err(&format!(
                "successfully decrypted with modified associated_data (i = {})",
                i
            ));
        // Restore the modified byte.
        mod_associated_data[i / 8] = b
    }

    // Truncate the ciphertext.
    for i in 1..ciphertext.len() {
        cipher
            .decrypt(&ciphertext[..(ciphertext.len() - i)], associated_data)
            .expect_err(&format!(
...
fn test_eta_empty_params() {
    ...
    cipher
        .encrypt(message, &[])
        .expect("encryption failed with empty associated_data");
    cipher
        .encrypt(&[], &[])
        .expect("encryption failed with empty ciphertext and associated_data");
}
